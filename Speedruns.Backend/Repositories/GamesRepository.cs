...
namespace Speedruns.Backend.Repositories
{
    public class GamesRepository : IGamesRepository
    {
        ...
        {
            return await _games.Include(x => x.GameConsoles).ThenInclude(x => x.Console).ToListAsync();
        }

        public async Task<GameModel> GetById(long id)
        {
            return await _games.Where(x => x.Id == id).Include(x => x.GameConsoles).ThenInclude(x => x.Console).FirstOrDefaultAsync();
        }

        public async Task<GameModel> GetByName(string name)
        {
            return await _games.Include(x => x.GameConsoles).ThenInclude(x => x.Console).FirstOrDefaultAsync(x => x.Name == name);
        }
